...
from nav_brain    import RoutePlanner, optimize_tsp, smooth_path, _lat, _lng, DEFAULT_RADIUS
...
st.markdown("""
<style>
[data-testid="stAppViewContainer"],[data-testid="stApp"],.main{background:#0b0b0f !important;}
[data-testid="stSidebar"]>div:first-child{background:#10101a !important;border-right:1px solid #1f1f2e !important;}
html,body,[class*="css"],p,span,label,div{color:#d4d4e0 !important;font-family:'Inter','Segoe UI',sans-serif !important;}
h1{font-size:1.9rem !important;font-weight:800 !important;color:#ffffff !important;letter-spacing:-1px;}
h2,h3{color:#c0c0d8 !important;font-weight:700 !important;}
div[data-testid="stButton"]>button[kind="primary"]{background:linear-gradient(135deg,#6d28d9 0%,#2563eb 100%) !important;color:#fff !important;border:none !important;border-radius:12px !important;font-size:1.08rem !important;font-weight:800 !important;padding:0.75rem 1.5rem !important;box-shadow:0 6px 24px rgba(109,40,217,.40) !important;}
...
div[data-testid="stButton"]>button:hover{background:#1e1e30 !important;border-color:#4040a0 !important;}
...
.stDownloadButton>button{background:#0f2318 !important;color:#6ee47a !important;border:1px solid #1a4028 !important;border-radius:9px !important;}
...
def _stop_bridge():
    ...
    _ur.urlopen(_ur.Request("http://127.0.0.1:7788/stop",
                            data=b'{}', headers={"Content-Type": "application/json"}), timeout=3)
...
def _generate_gpx_xml(loops=30, use_jitter=True):
    ...
    return minidom.parseString(ET.tostring(gpx, encoding='utf-8')).toprettyxml(indent="  ")
...
def _render_map(center_lat, center_lon):
    ...
    for p in st.session_state.selected_pois:
        if str(p.get('id','')).startswith('custom') or str(p.get('id','')).startswith('instant'):
            folium.CircleMarker(
                location=[_lat(p), _lng(p)], radius=8,
...
with col_title:
    st.markdown("<h1>🛸 Antigravity &nbsp;<span style='font-size:.5em;color:#5a5a7a;font-weight:400'>導航大腦 · 最終版</span></h1>",
                unsafe_allow_html=True)
with col_status:
    dot  = "🟢" if st.session_state.connected else "🔴"
    st.metric("裝置", f"{dot} {'連線中' if st.session_state.connected else '未連接'}")
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**📍 探索中心座標**")
    coord_text = st.text_input("座標", value=st.session_state.center_text,
                               placeholder="25.0339, 121.5644", label_visibility="collapsed")
    ...
    if instant_mode:
        ...
        st.caption("點擊地圖 → 直接跳到該座標" if instant_teleport else "點擊地圖 → 自動規劃並注入手機")
    else:
    ...
    st.divider()
    st.markdown("**🚶 模擬移動時速**")
    target_speed = st.slider("km/h", min_value=1, max_value=100, value=10, step=1,
    ...
    if st.button("📍 一鍵載入中原大學", use_container_width=True):
        ...
        st.session_state.map_center    = [24.95820, 121.24130]
    ...
    if st.button("⬜ 一鍵載入簡單矩形路線", use_container_width=True):
        ...
        st.session_state.map_center    = [25.02600, 121.54400]
    ...
    with st.expander("地毯式掃描花農座標", expanded=False):
        harvest_coord = st.text_input("中心座標", value="25.0339, 121.5644",
                                      placeholder="緯度, 經度", key="harvest_coord")
        harvest_lat, harvest_lng = _parse_coord(harvest_coord, default=(25.0339, 121.5644))
        harvest_radius = st.select_slider(
            "掃描半徑", options=[100, 200, 300, 500, 700, 1000], value=500,
            format_func=lambda x: f"{x}m" if x < 1000 else "1km"
        )
        harvest_mode = st.radio("路徑模式", ["🌀 螺旋掃描", "🔲 網格掃描"],
                                horizontal=True, key="harvest_mode")

        if st.button("🌸 生成收成路徑", use_container_width=True):
            points = []
            if "螺旋" in harvest_mode:
                turns = 8
                total_points = turns * 36
            ...
            st.success(f"✅ 生成 {len(points):,} 個掃描點！")
            st.rerun()
    ...
    gpx_loops  = st.slider("🔄 循環次數", min_value=1, max_value=500, value=30, step=10)
    ...
    if st.session_state.smooth_path:
        ...
        st.download_button(
            label=f"💾 導出循環 GPX ({gpx_loops} 圈)", data=gpx_data,
            file_name=f"antigravity_x{gpx_loops}.gpx", mime="application/gpx+xml",
            use_container_width=True
        )
    else:
        st.download_button(label="💾 導出循環 GPX", data="", disabled=True,
...
with col_ctrl:
    ...
    # ── 自訂座標 ──────────────────────────────────────────────────────────
    st.markdown("### 📍 自訂座標")
    with st.expander("手動輸入座標", expanded=False):
        custom_coord = st.text_input("座標（緯度, 經度）", value="24.9580, 121.2410",
                                     placeholder="24.9580, 121.2410", key="custom_coord")
        custom_name  = st.text_input("名稱", value="自訂點", key="custom_name")
        if st.button("➕ 加入路線", use_container_width=True):
            clat, clng = _parse_coord(custom_coord, default=(24.9580, 121.2410))
            new_poi = {
                'id': f'custom_{clat:.5f}_{clng:.5f}',
                'name': custom_name, 'lat': clat, 'lon': clng, 'category': 'custom',
            }
    ...
    with st.expander("直接跳到某個座標", expanded=False):
        tp_coord = st.text_input("座標（緯度, 經度）", value="24.9580, 121.2410",
                                  placeholder="24.9580, 121.2410", key="tp_coord")
        if st.button("🌀 立刻傳送到此座標", use_container_width=True):
            try:
                tp_lat, tp_lng = _parse_coord(tp_coord, default=(24.9580, 121.2410))
    ...
    if st.button("📡 注入至 iOS 裝置", disabled=not st.session_state.connected,
                 use_container_width=True):
        try:
...
with col_map:
    ...
    # 保存地圖當前視角
    if st_map and st_map.get("center"):
        c = st_map["center"]
        st.session_state.map_center = [c["lat"], c["lng"]]
    if st_map and st_map.get("zoom"):
